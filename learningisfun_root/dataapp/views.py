...
class PackagesTable(tables.Table):
    class Meta:
        model = Packages
        fields = ('package_name', 'stars', 'forks')

def display_data(request):
    # django_packages_page_data.json to df
    ...
    for _, row in merged_df.iterrows():
        table_data.append(Packages(**row.to_dict()))

    table = PackagesTable(table_data, template_name="django_tables2/bootstrap4.html")
    
    # max_value = max(merged_df['stars'])
    # min_value = min(merged_df['stars'])
